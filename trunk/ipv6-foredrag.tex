...
\subtitle{21 år, men har ennå ikke fått den største suksessen}
...
\begin{frame}%[allowframebreaks]
  ...
  \item ARP og RARP for IPv4 er en del av ICMPv6 for IPv6
    \begin{itemize}%[<+->]
...
\begin{frame}%[allowframebreaks]
  ...
  \item Dersom \href{http://www.afrinic.net/}{AFRINIC} oppfører seg
    ...
    holde på til \href{http://www.potaroo.net/tools/ipv4/}{5.\ 
      oktober 2018}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item \texttt{http:://[2001:db8:x:y::z]:portnummer/bla/bla}
  \item \texttt{https:://[2001:db8:x:y::z]/bla/bla}
  \item \texttt{https:://[2001:db8:x:y::z]:portnummer/bla/bla}
  \end{itemize}
\end{frame}
...
\end{Verbatim}
  ...
  \item Forespørselen ble sendt til «Solicited-node multicast-adressa»
    \texttt{FF02::1:FF\alert<7>{52}:\alert{67E2}}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Lurt å sette en fast link-local-adresse i routeren:
    \begin{enumerate}%[<+->]
  ...
  \item Deretter konfigureres WLAN-kontrolleren omtrent slik:
    \begin{enumerate}%[<+->]
    \item \texttt{config interface vlan management 100}
    \item \alert{\texttt{config ipv6 interface address management primary fd5c:14cf:c300:100::2 64 fe80::1}}
    \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item IPv6 er påskrudd i Windows Vista, Server 2008 og nyere versjoner
    ...
    \item WSUS-tjenester må ha \alert{IPv4} i tillegg til IPv6 for å
      kommunisere med Microsofts globale oppdateringsservere
    \end{itemize}
