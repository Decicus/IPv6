...
\subtitle{22 år, men ennå ikke den største suksessen}
...
\begin{frame}
  ...
  \item Har eksistert siden desember 1995, \rfc{1883}
  \item Endelig standardisert i \rfc{8200}, juli 2017
  ...
  \item Før eller siden blir CGN for kostbart og komplisert å vedlikeholde
    ...
    \item CGN gjør det mer komplisert å
      \href{http://www.nyteknik.se/nyheter/it_telekom/allmant/article3904264.ece}{spore
        abonnenter}
    \end{itemize}
  \item 4G/LTE og 5G klarer kanskje å øke IPv6-presset\hfill\prfc{6459}
...
\begin{frame}
  ...
  \item Adresserommene fra \rfc{1918}
    \begin{itemize}
  ...
  \item Microsoft eksperimenterte med ren IPv6 i 2016, og møtte noen hindre på veien
    \begin{itemize}
    \item routere som ikke støtter RDNSS \prfc{8106} og gjør livet
      surt for Android,
    \item Android som ikke støtter DHCPv6 \prfc{3315} og gjør livet
...
\begin{frame}
  ...
  \item Hierarkisk adressestruktur
    ...
    \item Kunden mottar som regel et /48-, /52- eller et
      /56-prefiks
    \item Kunden kan da opprette \(2^{128-48-64}=2^{16}=\underline{65536}\),
      \(2^{128-52-64}=2^{12}=\underline{4096}\) eller \(2^{128-56-64}=2^8=\underline{256}\)
      subnett (VLAN)
    \end{itemize}
...
\begin{frame}
  ...
  \item Flow Label-feltet kan også brukes til å smugle data sammen med
    ...
    \rfc{6437}
  \item Se \rfc{3595}, \rfc{6294}, \rfc{6436}, \rfc{6437} og \rfc{8200}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Utvidelsesheaderne finnes i stort antall:
    ...
    \end{enumerate}
  \item Se \rfc{4302}, \rfc{4303}, \rfc{6275} og \rfc{7045} og
    \rfc{8200}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Personal Area Networks (PAN): \rfc{4944}
  ...
  \item Nulladressa:
    ...
    \item \texttt{0:0:0:0:0:0:0:0/128} eller \texttt{::/128}
      ...
      \item Brukes av tjenester som godtar forespørsler fra alle
        ...
        \href{http://www.freebsd.org/cgi/man.cgi?query=bind&apropos=0&sektion=2&manpath=FreeBSD+11.1-RELEASE&arch=default&format=html}{\texttt{bind}(2)}-systemkallet
        i «Juniks»)
      \end{itemize}
...
\begin{frame}
  ...
  \item IPv4-mapped IPv6 addresses:
    ...
    \item Tjenestene må da åpne separate lyttesocketer for IPv4 og
      IPv6
    \end{itemize}
  \item \rfc{6890} og \rfc{8190} inneholder en oversikt over alle
...
\begin{frame}
  ...
  \item Problemet kan løses med «SEcure Neighbor Discovery» (SEND),
    \rfc{3971}, og \rfc{6494}, \rfc{6495} og \rfc{6980}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item \textbf{Ikke} bruk site-local-adresser \prfc{3879}
...
\begin{frame}
  ...
  \item \rfc{7371} utvider to av multicastadressetypene med et nytt
    4-bit flaggfelt
...
\begin{frame}[fragile]
  ...
  \begin{multicols}{2}
    \begin{itemize}
    \item Følgende rekkevidder er definert i \rfc{4921} og \rfc{7346}:
...
\ifratio{43}{% 4:3
\begin{frame}[shrink=12.73]
\multicastadresser%
\end{frame}%
}{%
\ifratio{169}{% 16:9
\begin{frame}[shrink=15.60]
\multicastadresser%
\end{frame}%
}{% 16:10
\begin{frame}
...
\section{Multicast Listener Report Version 2}
\begin{frame}
  \frametitle{ICMPv6}
...
\begin{frame}
  ...
  \item DHCPv6 er definert i \rfc{3315} med oppdateringer fra
    ...
    \rfc{6644}, \rfc{7083}, \rfc{7227}, \rfc{7283} og \rfc{7550}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Flaggene heter \texttt{XRPT} \prfc{7371}
  \item Flagget \texttt{T} angir med \texttt{0} at adressa er velkjent
    (definert av \href{http://www.iana.org/}{IANA}), og med \texttt{1}
    at adressa er midlertidig (lokalt definert)
  \item Flagget \texttt{P} angir med \texttt{1} at adressa inneholder
  ...
  \item \rfc{7371} delegerer 4 av de 8 reserverte bitene i
...
\end{Verbatim}
  \begin{itemize}
  \item Adresseformatet er gitt av \rfc{3306} og \rfc{7371}
  \item \alert<2>{De 12 mest signifikante bitene} må beholdes som vist
...
\end{Verbatim}
  \begin{itemize}
  \item Adresseformatet er gitt av \rfc{3956} og \rfc{7371}
  \item \alert<2>{De 12 mest signifikante bitene} må beholdes som vist
  \item \alert<3>{Rekkevidden} settes til ønsket, lovlig verdi, og
    rekkevidden skal ikke overskride \textit{utbredelsen\/} av det
    angitte nettverksprefikset
...
\begin{frame}
  ...
  \begin{itemize}
  \item IPv6-spesifikasjon: \rfc{8200}, \rfc{5095}, \rfc{5722},
...
\begin{frame}
  ...
  \item Grunnleggende krav til IPv6-routere hos sluttbrukere (CER):
    \rfc{7084}
  \item Valg av router når man har flere prefiks: \rfc{8028}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item «IPv6 Fundamentals: A Straightforward Approach to
    ...
    \ifratio{43}{}{ % Underpunktet tar for mye plass for 4:3-formatet.
      \begin{itemize}
