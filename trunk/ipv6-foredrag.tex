...
\begin{frame}%[allowframebreaks]
  ...
  \item Delegeringa er hittil bare gyldig når flagget \texttt{T} er
    påskrudd, eller når flaggene \texttt{PT} eller \texttt{RPT} er
    påskrudd
  \end{itemize}
\end{frame}
...
|   8    |  4 |  4 |  4 |  4 | 8  |       64       |    32    |
+--------+----+----+----+----+----+----------------+----------+
|11111111|0111|\Alert{2}{scop}|ff2 |\Alert{4}{RIID}|\Alert{3}{plen}| \Alert{3}{network prefix} | group ID |
+--------+----+----+----+----+----+----------------+----------+
\end{Verbatim}
  %pause
  \begin{itemize}%[<+->]
  \item Eksempel:
    \begin{itemize}%[<+->]
    \item \texttt{FF7\alert<2>{8}:0\alert<4>{1}\alert<3>{30}:\alert<3-4>{2001:700:1100:0}:1337:1337}
      \begin{itemize}%[<+->]
