...
% det av at det er noen frames før den aller første \part-kommandoen.
\makeatletter
...
\begin{frame}%[allowframebreaks]
  ...
  \item Høsten 2010: Enda en IPv4-serie ble innført:
    ...
    \begin{itemize}[<+->]
    \item \texttt{128.39.194.0/24} brukes nå til datalab med samme
      subnetting (inndeling) som den gamle
      \texttt{128.39.172.0/24}-serien
...
|   8    |  4 |  4 |   8    |    8   |       64       |    32    |
+--------+----+----+--------+--------+----------------+----------+
|\Alert{4}{11111111}|\Alert{4}{0011}|\Alert{5}{scop}|reserved|  \Alert{6}{plen}  | \Alert{7}{network prefix} | group ID |
+--------+----+----+--------+--------+----------------+----------+
\end{Verbatim}
  \pause
  \begin{itemize}[<+->]
  ...
  \item Feltet \alert<6>{«plen»} settes til prefikslengden til
    nettverksprefikset, \(0<\mathtt{plen}\le64\)
  \item \alert<7>{Nettverksprefikset} er ditt eget unicast-prefiks,
    paddet med null til det blir 64 bit langt
  \item «Group ID» settes i tråd med retningslinjene til \rfc{3307}
  \end{itemize}
\end{frame}
...
\end{Verbatim}
  ...
  \item Eksempel:
    \begin{itemize}[<+->]
    \item \texttt{FF3E:0030:2001:700:1100:0:1337:1337}
    \item Den første adressa er begrenset til internett (global, 48-bit)
    \item \texttt{FF38:0030:2001:700:1100:0:1337:1337}
    \item Den andre adressa er begrenset til FSI (organizational-local, 48-bit)
    \item \texttt{FF32:0040:2001:700:1100:3:1337:1337}
...
|   8    |  4 |  4 |  4 |  4 | 8  |       64       |    32    |
+--------+----+----+----+----+----+----------------+----------+
|\Alert{4}{11111111}|\Alert{4}{0111}|\Alert{5}{scop}|rsvd|\Alert{6,7}{RIID}|\Alert{8}{plen}| \Alert{9}{network prefix} | group ID |
+--------+----+----+----+----+----+----------------+----------+
\end{Verbatim}
  \pause
  \begin{itemize}[<+->]
  ...
  \item Feltet «RIID» settes til \alert<6>{møtepunktets
      grensesnittidentifikator}
    \begin{itemize}[<+->]
    \item \alert<7>{RIID} kan ikke være \(0\), for dette skaper
      konflikt med «Subnet-Router Anycast Address» fra \rfc{3513}
    \end{itemize}
  \item Feltet «plen» settes til \alert<8>{prefikslengden til
      nettverksprefikset}, \(0<\mathtt{plen}\le64\)
  \item \alert<9>{Nettverksprefikset} er ditt eget unicast-prefiks,
    paddet med null til det blir 64 bit langt
  \item «Group ID» settes i tråd med retningslinjene til \rfc{3307}
  \end{itemize}
\end{frame}
...
\end{Verbatim}
  ...
  \item Eksempler:
    \begin{itemize}[<+->]
    \item \texttt{FF7\alert<3>{8}:0\alert<5>{1}\alert<4>{30}:\alert<4-5>{2001}:\alert<4-5>{700}:\alert<4-5>{1100}:\alert<4-5>{0}:1337:1337}
      \begin{itemize}[<+->]
