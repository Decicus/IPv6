...
\begin{frame}%[allowframebreaks]
  ...
  \item Grensesnittidentifikatorer følger
    ...
    \item universal/local-bitet brukes med \textit{invertert\/}
      betydning/verdi
      \begin{itemize}[<+->]
      \item (gruppebitet mister sin vanlige betydning)
      \end{itemize}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Generér to autokonfigurerte IPv6-adresser:
    \begin{itemize}[<+->]
    \item Konstant grensesnittidentifikator
...
\begin{frame}%[allowframebreaks]
  ...
  \item \href{http://tools.ietf.org/html/rfc4941}{RFC~4941} angir
    ...
    \item Bruk de 64 \textit{mest\/} signifikante bitene og sett det
      sjette mest signifikante bitet til null (indikerer lokalgitt
      grensesnittidentifikator)
    ...
    \item Lagre de 64 \textit{minst\/} signifikante bitene fra trinn 2
      som historisk verdi for bruk den neste gangen algoritmen brukes
    \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Prefikset \texttt{\alert{FC}00::/\alert{8}} er reservert
    inntil videre
  \item \href{http://www.sixxs.net/}{SixXS} tilbyr:
    \begin{itemize}[<+->]
