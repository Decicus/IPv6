...
\begin{frame}%[allowframebreaks]
  ...
  \item \alert<17>{IPv6 er det eneste tilgjengelige og realistiske alternativet}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Grensesnittidentifikatorer er på 64 bit
    \begin{itemize}[<+->]
    \item Unntatt fra denne regelen er adresser som starter på $(000)_2$
    \end{itemize}
...
\begin{frame}%[allowframebreaks]
  ...
  \item \texttt{128} $=$ \texttt{0128} $=$ \texttt{000000\alert{0}100101000}\hfill(heks, heks, bin)
  \item Dette gikk bra!
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Routeren annonserer:
    ...
    \item Annonseringsflagg:
      ...
      \item Om DHCPv6 skal brukes for andre opplysninger
      \item «Home Agent» eller ikke «Home Agent»
      \item Preferansenivå for denne routeren, Low, Medium, eller High
      \end{itemize}
