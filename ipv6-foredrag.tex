...
\begin{frame}%[allowframebreaks]
  ...
  \item Høsten 2010: Enda en IPv4-serie ble innført:
    ...
    \end{itemize}
  \item Sommeren 2014: Tok i bruk nye linknett fordi \texttt{fig-gsw}
    ble tilkoblet \texttt{gjovik-gw1}
    \begin{itemize}[<+->]
    ...
    \item IPv6-linknett: \texttt{2001:700:0:8074::/64}
      \begin{itemize}[<+->]
      \item \texttt{2001:700:0:8074::1} brukes hos HiG
      \item \texttt{2001:700:0:8074::2} brukes hos FSI
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}%[allowframebreaks]
  ...
  \item Nulladressa:
    ...
    \item Brukes av tjenester som godtar forespørsler fra alle
      grensesnitt (\texttt{::/128}, sjekk ut
...
\end{Verbatim}
  ...
  \item Eksempler:
    ...
    \item \texttt{FF7\alert<3>{8}:0\alert<5>{1}\alert<4>{30}:\alert<4-5>{2001}:\alert<4-5>{700}:\alert<4-5>{1100}:\alert<4-5>{0}:8000:1337}
      ...
      \item Møtetpunktets adresse er \alert<6>{\texttt{\alert<5>{2001:700:1100}::\alert<5>{1}}}
      \item Møtetpunktets adresse må konfigureres på et
